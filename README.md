...
| S0027        | Determines if Thundering Herd is occuring                       | When hundreds of nodes check in simultaneously to request a catalog, it might cause a thundering herd of processes that causes CPU and memory performance to suffer. To verify that you have a thundering herd condition, run a query on the PuppetDB node to show how many nodes check in per minute. Query result can indicate whether your environment is affected with thundering herd or not. See                (https://support.puppet.com/hc/en-us/articles/215729277-Determine-a-thundering-herd-condition-in-Puppet-Enterprise-3-0-and-later)                                                                      | Raise a support case quoting reference S0027 along with the output of query result details specified in the KB article link.
## Limitations
