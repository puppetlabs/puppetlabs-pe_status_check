...
describe 'self_service class' do
  ...
  "memory": {
    "system": {
      ...
      it 'if S0041 conditions for false are met' do
        run_shell('puppet config set report false', expect_failures: false)
        result = run_shell('facter -p self_service.S0041')
        expect(result.stdout).to match(%r{false})
        run_shell('puppet config set report true', expect_failures: false)
      end
    end
  end
end
