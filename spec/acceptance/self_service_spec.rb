  ...
  "memory": {
    "system": {
      ...
      it 'if S0042 conditions for false are met' do
        run_shell('bash -c "touch /opt/puppetlabs/server/data/puppetdb/stockpile/cmd/q/{1..1010}.txt"')
        result = run_shell('facter -p self_service.S0042')
        expect(result.stdout).to match(%r{false})
        run_shell('rm -f /opt/puppetlabs/server/data/puppetdb/stockpile/cmd/q/*.txt')
      end
    end
  end
end
